...
catalog = {
	...
	initCategories : function() {
		var catalog = this.getParam(document.location.search)['name']
		$("#tit").html(catalog.split('_').join(' '))
	   	$.getJSON("/categories", function(data) {
	   	    data.map(function(item){
	   	    	var catalogName = item.split(' ').join('_')
	   	    	var html =''
	   	    	if (catalog === catalogName){
		   	    	 html = ' <li class="active"> <a href="/catalog?name=' + catalogName +'"> <i class="pe-7s-graph"></i> <p>' + item + '</p> </a> </li>'
	   	    	} else{
	   	    	     html = ' <li> <a href="/catalog?name=' + catalogName +'"> <i class="pe-7s-graph"></i> <p>' + item + '</p> </a> </li>'
	   	    	}
	   	    	$('#cats').append(html)
	   	    })
	   	})
	...
	/*	$.getJSON("/coll/m_status", function(data) {
		...
		//	Chartist.Pie('#chartLicense', dataPreferences, optionsPreferences);
	   	...
	   	$.getJSON("/catalogQuery/dataset_by_group/" + catalog, function(dataDist) {
	   ...
	   $.getJSON("/catalogQuery/ko/" + catalog, function(data) {	
		var tot = 0;
	    data.map(function(item){
	    tot = tot + 1
		var name = item.name
	...
	showNotification : function(from, align) {
		...
		$
				.notify(
						{
							icon : "pe-7s-gift",
							message : "Sito in versione <b>BETA</b> - ci scusiamo per eventuali errori"
